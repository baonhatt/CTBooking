...
async function main() {
  ...
  const out: string[] = [];
  out.push(`CREATE TABLE IF NOT EXISTS movies (id INTEGER PRIMARY KEY, title TEXT NOT NULL, description TEXT, cover_image TEXT, detail_images TEXT, genres TEXT, rating REAL, duration_min INTEGER, release_date TEXT, is_active INTEGER DEFAULT 1);`);
  ...
  for (const r of moviesRes.rows) {
    ...
    const releaseDateStr =
      r.release_date instanceof Date ? (r.release_date as Date).toISOString() : r.release_date ? String(r.release_date) : null;
