...
export function OTPDialog({ isOpen, onOpenChange, tempAccountId, email, onSuccess, auth }: OTPDialogProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.status === 'success' || data.status === 200) {
        // Mirror cookie để frontend check nhanh
        setCookie('userToken', data.token, 86400);
      ...
      <DialogContent className="bg-gradient-to-br z-[10000] from-[#0b1226] via-[#0e1b3d] to-[#050915] border border-cyan-500/30 text-white shadow-[0_0_50px_rgba(59,130,246,0.3)]">
        <DialogHeader>
          <div className="flex justify-center mb-4">
            <img src="/icon.svg" alt="CINESPHERE" className="h-16 w-16" />
          </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="text-center">
            <div className={cn('text-2xl font-mono font-bold', timeLeft <= 60 ? 'text-red-400' : 'text-cyan-400')}>
              {formatTime(timeLeft)}
            </div>
            <div className="text-xs text-gray-400 mt-1">{timeLeft <= 60 ? 'OTP sắp hết hạn' : 'OTP hết hạn sau'}</div>
          </div>
