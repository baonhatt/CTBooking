...
export default function UserPostDetailPage() {
        ...
        const processed = useMemo(() => ({ html: post?.content || '' }), [post?.content]);
        ...
        useEffect(() => {
                const run = async () => {
                        ...
                        setLoading(true);
                        try {
                                const p = await getPostBySlugId(slug as string);
                                ...
                                {loading ? (
                                        ...
                                        <div className="container mx-auto px-4 md:px-6 lg:px-8 mt-8">
                                                <div className="rounded-2xl border border-white/10 bg-white/5 p-8 text-slate-300">
                                                        ...
                                                        <Link href="/posts" className="text-cyan-300 underline">
                                                                                ...
                                                                                {recentPosts.length === 0 ? (
                                                                                        ...
                                                                                        <div className="mt-3 space-y-3">
                                                                                                {recentPosts.map((item) => (
                                                                                                        <Link key={item.id} href={`/posts/${item.id}`} className="block group">
                                                                ...
                                                                <div className="max-w-6xl mx-auto">
                                                                        ...
                                                                        <div className="mt-4 grid grid-cols-1 md:grid-cols-3 gap-4">
                                                                                {relatedPosts.map((item) => (
                                                                                        <Link
                                                                                                key={item.id}
                                                                                                href={`/posts/${item.slug ? `${item.slug}-${item.id}` : item.id}`}
