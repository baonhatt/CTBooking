...
export default function BookingPage() {
        ...
        const performBooking = async () => {
                try {
                        ...
                        // CS + 10 số timestamp (giây) + 2 số random = 12 số total
                        ...
                        const isBranchClosed = errorMessage.includes('đóng cửa') || errorMessage.includes('ngừng hoạt động');

                        toast.error(isBranchClosed ? 'Chi nhánh đã đóng cửa' : 'Không thể tạo đặt vé', {
                        ...
                        <div className="relative min-h-screen">
                                ...
                                <div className="relative z-10 max-w-6xl mx-auto p-4 pt-[104px] sm:pt-24 pb-24 lg:pb-0">
                                        <div className="text-sm pb-6 pt-2 flex items-center gap-1.5 opacity-80">
                                                <button className="text-gray-400 hover:text-blue-400 transition-colors" onClick={goHomeWithBranch}>
                                                        Trang chủ
                                                </button>
                                                                        ...
                                                                        <div className="lg:col-span-5 p-6 space-y-5 bg-black/10">
                                                                                ...
                                                                                <section className="space-y-3">
                                                                                        ...
                                                                                        <div className="grid grid-cols-1 gap-3 bg-white/5 p-4 rounded-xl border border-white/10">
                                                                                                ...
                                                                                                {formErrors.name && (
                                                                                                        <p className="text-orange-400 text-[10px] mt-1 animate-pulse">{formErrors.name}</p>
                                                                                                )}
                                                                                                <div className="space-y-1.5">
                                                                                                        ...
                                                                                                        {formErrors.phone && !phoneError && (
                                                                                                                <p className="text-orange-400 text-[10px] mt-1 animate-pulse">{formErrors.phone}</p>
                                                                                                        )}
                                                                                                </div>
                                                                                                <div className="space-y-1.5">
                                        ...
                                        {/* Sticky Action Bar for Mobile - Step 0 */}
                                        {!isLoadingPage && step === 0 && selectedPackage && (
                                                <div className="lg:hidden fixed bottom-0 left-0 right-0 z-[40] p-4 bg-gradient-to-t from-black via-black/95 to-transparent border-t border-white/10">
                                                        <div className="flex items-center justify-between gap-4">
                                                                <div className="flex-1 min-w-0">
                                                                        <p className="text-[10px] text-gray-400 uppercase font-bold truncate">Tạm tính ({ticketCount} vé)</p>
                                                                        <p className="text-lg font-black text-blue-400 leading-tight">
                                                                                {totalPrice.toLocaleString('vi-VN')}₫
                                                                        </p>
                                                                </div>
                                                                <Button
                                                                        className="px-8 h-12 bg-gradient-to-r from-blue-600 to-blue-500 text-white font-bold rounded-xl shadow-lg active:scale-95 transition-transform"
                                                                        onClick={() => setStep(1)}
                                                                >
                                                                        Tiếp tục
                                                                </Button>
                                        ...
                                        {/* Sticky Action Bar for Mobile - Step 1 */}
                                        {!isLoadingPage && step === 1 && (
                                                <div className="lg:hidden fixed bottom-0 left-0 right-0 z-[40] p-4 bg-gradient-to-t from-black via-black/95 to-transparent border-t border-white/10">
                                                        <div className="flex items-center justify-between gap-4">
                                                                <div className="flex-1 min-w-0">
                                                                        <p className="text-[10px] text-gray-400 uppercase font-bold truncate">Tổng thanh toán</p>
                                                                        <p className="text-lg font-black text-blue-400 leading-tight">
                                                                                {totalPrice.toLocaleString('vi-VN')}₫
                                                                        </p>
                                                                </div>
                                                                <Button
                                                                        className="px-8 h-12 bg-gradient-to-r from-blue-600 to-blue-500 text-white font-bold rounded-xl shadow-lg disabled:opacity-50 active:scale-95 transition-transform"
                                                                        disabled={!confirmChecked || isProcessing}
                                                                        onClick={handleCreateAndPay}
                                                                >
                                                                        {isProcessing ? <Loader2 className="w-5 h-5 animate-spin" /> : 'Thanh toán'}
                                                                </Button>
                                                        </div>
                                                </div>
                                        )}
                                ...
                                <AlertDialogContent className="bg-[#0f172a] border-white/10 text-white">
                                        <AlertDialogHeader>
                                                ...
                                                <AlertDialogDescription className="text-gray-300">
                                                        Bạn có chắc chắn muốn đặt vé cho chi nhánh <span className="font-bold text-blue-400">{selectedBranch?.name}</span> và chuyển sang trang thanh toán không?
                                                </AlertDialogDescription>
                                        </AlertDialogHeader>
                                        <AlertDialogFooter>
                                                <AlertDialogCancel className="bg-white/5 border-white/10 text-gray-300 hover:bg-white/10 hover:text-white">
