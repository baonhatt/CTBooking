...
export function formatBranchIdsLabel(branch_ids: unknown, branches: { id: number; name: string }[]): string {
  ...
  const names = normalized!.map((id) => branches.find((b) => b.id === id)?.name).filter(Boolean);
