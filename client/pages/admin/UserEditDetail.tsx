...
import { ArrowLeft, Save, Shield, User, Mail, Clock, Activity } from 'lucide-react';
import { getUserById, updateAdminUser } from '@/lib/api/users';
...
export default function UserEditDetailPage() {
  ...
  const loadUserData = async () => {
    try {
      ...
      setFormData({
        ...
        email: data.email || '',
        role: (data.role as 'admin' | 'viewer' | 'user') || 'user'
      });
      ...
      toast.error('Lỗi', {
        description: err?.message || 'Không thể tải thông tin người dùng'
      });
    } finally {
      setIsLoading(false);
    }
        ...
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-slate-500">Quản lý thông tin và phân quyền</p>
            </div>
          </div>
          <Button
            onClick={handleSave}
