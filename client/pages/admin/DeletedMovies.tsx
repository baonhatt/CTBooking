              ...
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead
                    ...
                    onClick={() => {
                      setSortField('title');
                      setSortDir(sortField === 'title' && sortDir === 'asc' ? 'desc' : 'asc');
                    }}
                  >
                  ...
                  <TableHead
                    ...
                    onClick={() => {
                      ...
                      setSortDir(sortField === 'deleted_at' && sortDir === 'asc' ? 'desc' : 'asc');
                    }}
                  >
              ...
              <TableBody>
                {movies.map((m) => (
                  <TableRow key={m.id}>
                    <TableCell>
                      {m.cover_image ? (
                        <img src={m.cover_image} alt={m.title} className="w-12 h-16 object-cover rounded" />
                      ) : (
                    ...
                    <TableCell>{m.rating ? <Badge variant="outline">⭐ {m.rating}</Badge> : '-'}</TableCell>
                    <TableCell>{m.release_date ? new Date(m.release_date).toLocaleDateString('vi-VN') : '-'}</TableCell>
                    <TableCell className="text-gray-600">{m.deleted_by_staff_name || '-'}</TableCell>
                    <TableCell>{m.deleted_at ? new Date(m.deleted_at).toLocaleString('vi-VN') : '-'}</TableCell>
                    <TableCell className="text-right">
                      {hasPermission('movies', 'restore') && (
                        <Button onClick={() => handleRestore(m.id)} variant="outline" size="sm">
      ...
      <AlertDialog open={restoreDialogOpen} onOpenChange={setRestoreDialogOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>Bạn có chắc chắn muốn khôi phục phim này?</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
