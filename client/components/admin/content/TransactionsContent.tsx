...
export default function TransactionsContent({
  ...
  const resolvedPageSize = useMemo(() => {
    ...
    if (transactionsLength > 0 && totalPages > 0) return Math.max(1, Math.ceil(transactionsLength / totalPages));
    return 10;
  }, [pageSize, totalPages, transactionsLength]);
  ...
  const columns: ColumnsType<Tx> = useMemo(
    () => [
      ...
      },
      {
        ...
        render: (_: any, t: Tx) => (
          <div className="flex flex-col min-w-0">
            <span className="font-semibold text-sm text-gray-900 truncate" title={t.userName || 'Khách Vãng Lai'}>
              {t.userName || 'Khách Vãng Lai'}
            </span>
            <span className="text-[11px] text-gray-500 truncate" title={t.email}>
              {t.email}
            </span>
          </div>
        )
      ...
      {
        title: 'Sản phẩm',
        dataIndex: 'ticket_package_name',
        key: 'ticket_package_name',
        width: 260,
        ellipsis: true,
        render: (v) => (
          <span className="font-medium text-sm truncate block" title={String(v || '')}>
            {v}
          </span>
        )
      },
      ...
      },
      {
        ...
        key: 'totalPrice',
        width: 150,
        render: (v) => <span className="font-bold text-blue-700">{Number(v || 0).toLocaleString('vi-VN')}đ</span>
      },
      {
        title: 'Thời gian',
        key: 'time',
      ...
      },
      {
        ...
        ellipsis: true,
        render: (v) => (
          <span className="text-sm font-bold uppercase text-slate-700 truncate block" title={String(v || '')}>
            {v}
          </span>
        )
      },
