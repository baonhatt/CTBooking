                                ...
                                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                                        <div className="flex items-center gap-4">
                                                ...
                                                <div className="flex flex-col gap-1">
                                                        ...
                                                        <p className="text-xs text-slate-500 font-bold uppercase tracking-widest">Ghi lại tất cả thông báo gửi tới khách hàng</p>
                                                </div>
                                        </div>
                                                ...
                                                <TableBody>
                                                        {isLoading && data.length === 0
                                                                ...
                                                                : data.length === 0 ? (
                                                                        ...
                                                                        <TableRow key={log.id} className="group hover:bg-slate-50/80 transition-colors border-b last:border-0 h-20">
                                                                                ...
                                                                                <TableCell>
                                                                                        <div className="flex items-center gap-2">
                                                                                                ...
                                                                                                <span className="text-xs font-bold text-slate-700 uppercase tracking-tight">
                                                                                                        {log.email_type === 'welcome' ? 'Chào mừng' :
                                                                                                                log.email_type === 'reset_password' ? 'Quên mật khẩu' :
                                                                                                                        log.email_type === 'booking_confirmation' ? 'Xác nhận đặt vé' : log.email_type}
                                                                                                </span>
                                                                                        </div>
                                                                                </TableCell>
                                ...
                                <DialogContent className="max-w-2xl p-0 border-none shadow-2xl rounded-3xl flex flex-col overflow-hidden">
                                        <DialogHeader className="p-8 bg-slate-900 text-white shrink-0 relative overflow-hidden">
                                                ...
                                                <div className="relative z-10 flex flex-col gap-4">
                                                        ...
                                                        <div>
                                                                <DialogTitle className="text-xl font-black text-white leading-tight">Chi tiết Email Log</DialogTitle>
                                                                <p className="text-xs text-slate-400 font-bold uppercase tracking-widest mt-1">ID: #{selectedLog?.id}</p>
                                                        </div>
                                                </div>
                                        </DialogHeader>
                                                        ...
                                                        <div className="space-y-8">
                                                                ...
                                                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                                                        ...
                                                                        <div className="p-4 bg-slate-50 rounded-2xl border border-slate-100 flex flex-col gap-1">
                                                                                ...
                                                                                <span className="font-bold text-slate-900 uppercase tracking-tighter">{selectedLog.email_type}</span>
                                                                        </div>
                                                                </div>

                                                                {/* Connection & IDs */}
                                                                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                                                        <div className="p-4 bg-slate-900 rounded-2xl border border-slate-800 flex flex-col gap-1">
                                                                                <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Provider</span>
                                                                                <span className="font-bold text-blue-400 uppercase tracking-tight">{selectedLog.provider || "System"}</span>
                                                                        </div>
                                                                        <div className="p-4 bg-slate-50 rounded-2xl border border-slate-100 flex flex-col gap-1">
                                                                                <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">User ID</span>
                                                                                <span className="font-bold text-slate-900">
                                                                                        {selectedLog.user_id ? `#${selectedLog.user_id}` : <span className="text-slate-400 italic font-medium">Khách vãng lai</span>}
                                                                                </span>
                                                                        </div>
                                                                        <div className="p-4 bg-slate-50 rounded-2xl border border-slate-100 flex flex-col gap-1">
                                                                                <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Booking ID</span>
                                                                                <span className="font-bold text-slate-900">{selectedLog.booking_id ? `#${selectedLog.booking_id}` : "-"}</span>
                                                                        </div>
                                                                </div>
                                                                ...
                                                                {selectedLog.metadata && (
                                                                        <div className="space-y-3">
                                                                                <div className="flex items-center gap-2 text-slate-400">
                                                                                        <FileJson size={16} />
                                                                                        <span className="text-[10px] font-black uppercase tracking-widest">Dữ liệu đi kèm (Metadata)</span>
                                                                                </div>
                                                                                <div className="p-5 bg-slate-900 rounded-3xl text-blue-400 text-xs font-mono overflow-hidden shadow-inner">
                                                                ...
                                                                <div className="grid grid-cols-2 gap-4 pt-4 border-t border-slate-100">
                                                                        ...
                                                                        <div className="flex flex-col gap-1">
                                                                                <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest text-right">Gửi thành công lúc</span>
                                                                                <span className="text-xs font-bold text-slate-600 text-right">{formatDateTime(selectedLog.sent_at)}</span>
                                                                        </div>
                                                                </div>
                                                        </div>
                                                )}
