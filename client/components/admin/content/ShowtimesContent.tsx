...
import { CalendarClock, ChevronRight, Clock, Copy, Loader2, MapPin, Pencil, Plus, RefreshCw, Trash2, X } from 'lucide-react';
...
export default function ShowtimesContent({ branches, schedules, isLoading, onRefresh, onRefreshBranch }: Props) {
        ...
        const resetForm = (next = false) => {
                const start = next && lastSlot?.end_time ? lastSlot.end_time : items.length === 0 ? '10:00' : lastSlot?.end_time || '10:00';
                setEditing(null);
                                        ...
                                        <DialogHeader className="flex flex-row items-center gap-3 space-y-0 pb-4 border-b shrink-0">
                                                <div>
                                                        <DialogTitle className="text-lg font-bold text-slate-800">
                                                                Lịch chiếu · {editorBranch?.name}
                                                        </DialogTitle>
                                                        <p className="text-xs text-slate-400 mt-0.5">Thêm lần lượt các suất trong ngày. Suất mới sẽ tự điền giờ bắt đầu bằng giờ kết thúc suất trước.</p>
                                                </div>
                                                        ...
                                                        <div className="rounded-xl border border-slate-200 bg-slate-50/70 p-4 space-y-3">
                                                                <div className="flex items-center justify-between gap-2">
                                                                        <p className="text-sm font-semibold text-slate-700">
                                                                                {editing ? 'Sửa suất' : 'Thêm suất'}
                                                                        </p>
                                                                        {editing && (
                                                                                <button type="button" onClick={() => resetForm(true)} className="text-xs text-slate-500 hover:text-slate-700">
                                                                                        Hủy sửa
                                                                                </button>
                                                                        )}
                                                                </div>
                                                                <div className="grid grid-cols-1 sm:grid-cols-4 gap-3">
                                                                        ...
                                                                        <div className="space-y-1.5">
                                                                                ...
                                                                                <Input type="time" value={form.start_time} onChange={(e) => handleStartChange(e.target.value)} className="h-10" />
                                                                        </div>
                                                                        <div className="space-y-1.5">
                                                ...
                                                <div className="rounded-xl border border-slate-200 overflow-hidden">
                                                        {items.length === 0 ? (
                                                                <div className="flex flex-col items-center justify-center py-12 text-center px-6">
                                                                        ...
                                                                        <p className="text-sm text-slate-400 mt-1">Thêm suất phía trên, hoặc sao chép lịch từ chi nhánh khác.</p>
                                                                </div>
                                                        ) : (
                                                                <div className="divide-y divide-slate-100">
                                                                        ...
                                                                        {timeline.map((row) => {
                                                                                if (row.type === 'gap') {
                                                                                        return (
                                                                                                <div key={row.key} className="px-4 py-1.5 text-[11px] font-medium text-amber-600 bg-amber-50/70">
                                                                                        ...
                                                                                        <div key={slot.id} className="flex items-center gap-3 px-4 py-3">
                                                                                                ...
                                                                                                <div className="flex items-center gap-1 shrink-0">
                                                                                                        {hasPermission('showtimes', 'edit') && (
                                                                                                                <Button variant="ghost" size="icon" className="h-8 w-8 text-slate-500" title="Sửa" onClick={() => openEdit(slot)}>
                        ...
                        <Dialog open={!!deleteTarget} onOpenChange={(open) => !open && setDeleteTarget(null)}>
                                <DialogContent className="[&>button]:hidden z-[60]">
                                        ...
                                        <DialogFooter>
                                                ...
                                                <Button onClick={handleDelete} disabled={isSaving} className="bg-red-600 hover:bg-red-700 min-w-[120px] rounded-xl">
