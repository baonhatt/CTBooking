...
interface Props {
  ...
  dateRevenue: { total: number; count: number; revenueByMethod: { cash: number; momo: number; vnpay: number; vietqr: number } };
          ...
          {segments.map(seg => (
            <div key={seg.key} className="flex items-center gap-3">
              ...
              <div className="flex flex-col">
                <span className="text-[10px] font-bold text-slate-500 uppercase tracking-widest leading-none mb-0.5">{seg.key}</span>
    ...
    <Card className="bg-white border-none shadow-slate-200/50 shadow-lg hover:shadow-xl transition-all duration-300 rounded-2xl overflow-hidden group">
      <CardContent className="p-6 relative">
        <div className="flex items-start justify-between relative z-10">
          <div className="space-y-1">
            ...
            {isPageLoading ? (
              ...
              <div className="flex items-baseline gap-2">
                ...
                {trend && (
                  <span className={`text-[10px] font-bold flex items-center ${trend > 0 ? 'text-emerald-500' : 'text-slate-400'}`}>
          ...
          <div className={`p-3 rounded-2xl group-hover:scale-110 transition-transform duration-300`} style={{ backgroundColor: `${color}15` }}>
    ...
    <div className="space-y-8 p-1 font-sans">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white p-6 rounded-2xl border shadow-sm">
        <div className="space-y-1">
           ...
           <p className="text-xs text-slate-500 font-medium pl-4 uppercase tracking-[0.2em]">Dữ liệu đã thanh toán • Cập nhật trực tiếp</p>
        </div>
        <div className="flex items-center gap-3">
           ...
           >
              <RefreshCcw size={18} className={`${isPageLoading ? 'animate-spin text-blue-600' : 'group-hover:rotate-180 transition-transform duration-500'}`} />
           </button>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <StatCard title={`Quà / Đồ chơi mới (${selectedYear})`} value={metrics.totalToys} icon={Package} color={colors.secondary} />
        <StatCard title={`Khách hàng mới (${selectedYear})`} value={metrics.totalUsers} icon={Users} color={colors.accent} />
        <StatCard title={`Giao dịch (${selectedYear})`} value={metrics.totalTransactions} icon={CreditCard} color={colors.teal} />
      </div>
             ...
             {dateFilterType !== "year" && (
                <div className="flex flex-wrap gap-4 items-end mb-8 animate-in fade-in slide-in-from-top-4 duration-300 bg-slate-50/50 p-4 rounded-2xl border border-slate-100/50">
                   <div className="flex-1 min-w-[200px]">
                      <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-2 pl-1">CHỌN THỜI GIAN</p>
             ...
             <div className="flex flex-col md:flex-row items-center justify-between gap-12 py-4 min-h-[160px]">
                <div className="space-y-2 text-center md:text-left">
                   <p className="text-[11px] font-bold text-slate-400 uppercase tracking-[0.3em]">
                      Của {dateFilterType === 'day' ? 'HÔM NAY' : dateFilterType === 'month' ? 'THÁNG NÀY' : `NĂM ${selectedYear}`}
                   </p>
                    <p className="text-4xl font-black text-slate-900 tracking-tight">
                       {isRevenueLoading ? <Skeleton className="h-12 w-48 rounded-xl" /> : `${dateRevenue.total.toLocaleString()} đ`}
                    </p>
                   <p className="text-xs font-bold text-blue-600 bg-blue-50 inline-flex px-3 py-1 rounded-full uppercase tracking-widest">{dateRevenue.count} Giao dịch đã xác thực</p>
                </div>
        ...
        <Card className="bg-white border-none shadow-slate-200/50 shadow-xl rounded-2xl flex flex-col relative">
          ...
          <CardHeader className="p-6 border-b border-slate-50 space-y-4">
            <div className="flex items-center justify-between">
               ...
               <span className="text-[10px] font-black bg-amber-100 text-amber-700 px-2 py-1 rounded-md uppercase tracking-tighter">THEO {topPeriod === 'week' ? 'TUẦN' : topPeriod === 'month' ? 'THÁNG' : 'NĂM'}</span>
            </div>
             ...
             <div className="space-y-2">
                {isTopTicketsLoading ? (
                  ...
                  metrics.topTicketsWeek.map((pkg, idx) => (
                    <div key={idx} className="group flex items-center gap-4 p-3 rounded-xl hover:bg-slate-50 transition-all cursor-default border border-transparent hover:border-slate-100">
                       ...
                       <div className="flex-1 min-w-0">
                          <p className="text-sm font-black text-slate-800 truncate group-hover:text-blue-600 transition-colors uppercase tracking-tight">{pkg.title}</p>
                          <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">Doanh thu: {pkg.revenue.toLocaleString()} đ</p>
                       </div>
                       <div className="text-right">
                          <p className="text-xs font-black text-slate-900">{pkg.count} <span className="text-[9px] text-slate-400">VÉ</span></p>
         ...
         <Card className="bg-white border-none shadow-slate-200/50 shadow-xl rounded-2xl flex flex-col overflow-hidden relative">
            ...
            <div className="p-6 bg-blue-600 text-white flex justify-between items-center">
               <div>
                  ...
                  <p className="text-[10px] text-blue-100 font-bold uppercase tracking-widest">Dựa trên doanh thực thực tế</p>
               </div>
            ...
            <div className="divide-y divide-slate-50 flex-1 flex flex-col min-h-[350px]">
               {metrics.topVipUsers?.length === 0 ? (
                  ...
                  metrics.topVipUsers?.map((user, idx) => (
                     <div key={user.userId} className="p-4 flex items-center justify-between hover:bg-slate-50 transition-colors group">
                        <div className="flex items-center gap-4">
                           ...
                           <div>
                              ...
                              <div className="flex items-center gap-2">
                                 <span className="text-[9px] font-black text-slate-400 uppercase tracking-tighter">{user.bookingCount} Giao dịch</span>
                              </div>
                           </div>
                        </div>
                        <div className="text-right">
                           ...
                           <div className="flex items-center justify-end gap-1 mt-0.5">
                              <span className="text-[9px] font-bold text-blue-500 uppercase tracking-tighter">Doanh thu đạt</span>
                           </div>
                        </div>
                     </div>
                  ))
         ...
         <Card className="bg-white border-none shadow-slate-200/50 shadow-xl rounded-2xl flex flex-col overflow-hidden relative">
            ...
            <div className="p-6 bg-slate-900 text-white flex justify-between items-center">
               <div>
                  ...
                  <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">Chỉ số hiệu suất thời gian thực</p>
               </div>
            ...
            <div className="p-6 space-y-8 flex-1">
               ...
               <div className="space-y-4">
                  <div className="flex justify-between items-end">
                     <div>
                        <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Tỷ lệ sử dụng vé</p>
                        <h4 className="text-lg font-black text-slate-900">
                           {metrics.ticketUsage.used} / {metrics.ticketUsage.total} <span className="text-[10px] text-slate-400 font-bold uppercase">Vé đã quét</span>
                        </h4>
                     </div>
                     <span className="text-2xl font-black text-blue-600">
                        {metrics.ticketUsage.total > 0 ? Math.round((metrics.ticketUsage.used / metrics.ticketUsage.total) * 100) : 0}%
                     </span>
                  </div>
                  <div className="h-2 w-full bg-slate-100 rounded-full overflow-hidden flex">
                     <div 
                        className="h-full bg-blue-600 transition-all duration-1000 ease-out" 
                        style={{ width: `${metrics.ticketUsage.total > 0 ? (metrics.ticketUsage.used / metrics.ticketUsage.total) * 100 : 0}%` }}
                     />
                  </div>
               </div>
               ...
               <div className="space-y-4">
                  ...
                  <div className="flex gap-1 h-3 rounded-full overflow-hidden">
                     {(() => {
                        const total = metrics.paymentHealth.paid + metrics.paymentHealth.pending + metrics.paymentHealth.failed;
                        if (total === 0) return <div className="w-full bg-slate-100" />;
                        return (
                           <>
                              <div className="bg-emerald-500 transition-all duration-500" style={{ width: `${(metrics.paymentHealth.paid / total) * 100}%` }} title="Thành công" />
                              <div className="bg-amber-400 transition-all duration-500" style={{ width: `${(metrics.paymentHealth.pending / total) * 100}%` }} title="Chờ xử lý" />
                              <div className="bg-rose-500 transition-all duration-500" style={{ width: `${(metrics.paymentHealth.failed / total) * 100}%` }} title="Thất bại" />
                           </>
                        );
                     })()}
                  </div>
                  <div className="grid grid-cols-3 gap-2">
                     <div className="flex items-center gap-1.5">
                        ...
                        <span className="text-[9px] font-black text-slate-600 uppercase">Paid: {metrics.paymentHealth.paid}</span>
                     </div>
                     <div className="flex items-center gap-1.5">
                        <div className="w-2 h-2 rounded-full bg-amber-400" />
                        <span className="text-[9px] font-black text-slate-600 uppercase">Pend: {metrics.paymentHealth.pending}</span>
                     </div>
                     <div className="flex items-center gap-1.5">
                        <div className="w-2 h-2 rounded-full bg-rose-500" />
                        <span className="text-[9px] font-black text-slate-600 uppercase">Fail: {metrics.paymentHealth.failed}</span>
                     </div>
                  </div>
               </div>
               ...
               <div className="space-y-4 pt-2">
                  <div className="flex items-center justify-between">
                     <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Khung giờ đặt vé (Heatmap 24h)</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-1 gap-8 pt-4">
         ...
         <Card className="bg-white border-none shadow-slate-200/50 shadow-xl rounded-2xl p-8 space-y-8">
            <div className="flex justify-between items-center">
               <div>
                  <h3 className="text-xl font-black text-slate-900 tracking-tight uppercase">Performance 7 Ngày ({selectedYear})</h3>
                  <p className="text-xs text-slate-400 font-bold uppercase tracking-widest pl-1">Phân tích hiệu suất doanh thu ngắn hạn</p>
               </div>
               <Badge className="bg-emerald-100 text-emerald-700 border-none shadow-none text-[10px] font-bold rounded-lg px-3 py-1 uppercase">Live Monitoring</Badge>
            </div>
         ...
         <Card className="bg-white border-none shadow-slate-200/50 shadow-xl rounded-2xl p-8 space-y-8">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
               <div className="space-y-1">
                  <h3 className="text-xl font-black text-slate-900 tracking-tight uppercase">Thống kê tăng trưởng năm {selectedYear}</h3>
                  <p className="text-xs text-slate-400 font-bold uppercase tracking-widest pl-1">Phân tích dòng tiền theo tháng</p>
               </div>
            </div>
            <BarChart
