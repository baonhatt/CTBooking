...
async function bootstrapSuperAdminIfNeeded(db: any, env: any, loginEmail: string) {
  const seedEmail = String(env?.SUPER_ADMIN_EMAIL || '')
    .trim()
    .toLowerCase();
  if (!seedEmail) return;

  if (loginEmail.trim().toLowerCase() !== seedEmail) return;

  const existing = await db.query.accounts.findFirst({ where: eq(schema.accounts.email, seedEmail) });
  if (existing) return;

  const seedPassword = String(env?.SUPER_ADMIN_PASSWORD || '').trim();
  if (!seedPassword) return;

  const seedName = String(env?.SUPER_ADMIN_NAME || 'Super Admin');
  const now = new Date().toISOString();
  const passwordHash = await bcrypt.hash(seedPassword, 10);

  const insertedUsers = await db
    .insert(schema.users)
    .values({ fullname: seedName, role: 'super_admin', created_at: now, updated_at: now })
...
// Admin: get current session user + permissions
// GET /api/admin/me
app.get('/api/admin/me', requireAuth, async (c) => {
  const user = (c as any).get('adminUser') as any;
...
// Admin: list all permissions (grouped)
// GET /api/admin/permissions
app.get('/api/admin/permissions', requireAuth, superAdminOnly, async (c) => {
  try {
    const db = drizzle(c.env.cinema_db, { schema });
    await ensurePermissionsSeeded(db);

    const rows = await db
      .select({ key: schema.permissions.key, name: schema.permissions.name, group: schema.permissions.group })
...
async function getPermissionIdsByKeys(db: any, keys: string[]): Promise<Array<{ id: number; key: string }>> {
  ...
  const rows = await db
    ...
    .from(schema.permissions)
    .where(
      sql`${schema.permissions.key} in (${sql.join(
        unique.map((k) => sql`${k}`),
        sql`,`
      )})`
    );
...
async function replaceUserPermissions(db: any, userId: number, permissionKeys: string[], grantedBy: number) {
  await ensurePermissionsSeeded(db);
  const now = new Date().toISOString();
  const permRows = await getPermissionIdsByKeys(db, permissionKeys);
  const foundKeys = new Set(permRows.map((r) => r.key));
  const invalid = permissionKeys.filter((k) => !foundKeys.has(k));
  if (invalid.length) {
    return { ok: false, message: `Permission không hợp lệ: ${invalid.join(', ')}` };
  }
...
app.post('/api/admin/login', async (c) => {
  try {
    const db = drizzle(c.env.cinema_db, { schema });
    const body = await c.req.json().catch(() => ({}));
    const emailForBootstrap = String((body as any)?.email || '');

    await ensurePermissionsSeeded(db);
    await bootstrapSuperAdminIfNeeded(db, c.env, emailForBootstrap);
...
app.get('/api/admin/users', requireAuth, superAdminOnly, async (c) => {
  try {
    const page = Number(c.req.query('page') || 1);
    const pageSize = Number(c.req.query('pageSize') || 20);
    ...
    const whereCondition = q
      ? or(
          sql`LOWER(${schema.users.fullname}) LIKE ${`%${q.toLowerCase()}%`}`,
          sql`LOWER(${schema.accounts.email}) LIKE ${`%${q.toLowerCase()}%`}`,
          sql`LOWER(${schema.users.phone}) LIKE ${`%${q.toLowerCase()}%`}`
        )
      : undefined;

    const permCountSq = db
      .select({
        userId: schema.user_permissions.user_id,
        count: count(schema.user_permissions.permission_id).as('count')
      })
      .from(schema.user_permissions)
      .groupBy(schema.user_permissions.user_id)
      .as('perm_counts');

    const items = await db
      .select({
        user: schema.users,
        email: sql<string>`max(${schema.accounts.email})`.as('email'),
        is_active_agg: sql<number>`max(CASE WHEN ${schema.accounts.is_active} THEN 1 ELSE 0 END)`.as('is_active_agg'),
        permissions_count: sql<number>`coalesce(max(${permCountSq.count}), 0)`.as('permissions_count')
      })
      .from(schema.users)
      .innerJoin(schema.accounts, eq(schema.users.id, schema.accounts.user_id))
      .leftJoin(permCountSq, eq(schema.users.id, permCountSq.userId))
    ...
    const [totalRes] = await db
      .select({ count: sql<number>`count(distinct ${schema.users.id})`.as('count') })
      .from(schema.users)
      .innerJoin(schema.accounts, eq(schema.users.id, schema.accounts.user_id))
      .where(whereCondition);
    ...
    const mapped = items.map((item: any) => ({
      ...
      email: item.email || '',
      role: item.user.role || 'admin',
      is_active: (item.is_active_agg ?? 1) > 0,
      permissions_count: Number(item.permissions_count || 0),
      created_at: item.user.created_at,
      updated_at: item.user.updated_at
    }));

    return c.json({ data: mapped, total: totalRes?.count || 0, page });
  } catch (err: any) {
    return c.json({ status: 'error', message: String(err?.message || 'Internal error') }, 500);
  }
});
...
app.post('/api/admin/settings', requireAuth, async (c) => {
  try {
    const body = await c.req.json().catch(() => ({}));
...
app.get('/api/admin/users/:id', requireAuth, superAdminOnly, async (c) => {
  try {
    const id = Number(c.req.param('id'));
    const db = drizzle(c.env.cinema_db, { schema });
    ...
    if (!userRow) return c.json({ status: 'error', message: 'Không tìm thấy' }, 404);

    const account = userRow.accounts?.[0];
    const permissions =
      (userRow as any).permissions?.map((up: any) => up?.permission?.key).filter((x: any) => typeof x === 'string') ||
      [];
...
app.post('/api/admin/users', requireAuth, superAdminOnly, async (c) => {
  try {
    const db = drizzle(c.env.cinema_db, { schema });
    const body = await c.req.json().catch(() => ({}));
    ...
    const role = String((body as any)?.role || 'admin');
    const permissions = Array.isArray((body as any)?.permissions) ? (body as any).permissions : [];

    if (!name || !email || !password) {
      return c.json({ status: 'error', message: 'Thiếu thông tin bắt buộc' }, 400);
    }

    if (!['admin', 'viewer', 'user'].includes(role)) {
      return c.json({ status: 'error', message: 'Role không hợp lệ' }, 400);
    }

    const exists = await db.query.accounts.findFirst({ where: eq(schema.accounts.email, email) });
    if (exists) {
      return c.json({ status: 'error', message: 'Email đã tồn tại' }, 400);
    }

    const now = new Date().toISOString();
    const passwordHash = await bcrypt.hash(password, 10);
    const adminUser = (c as any).get('adminUser') as any;
    const createdBy = Number(adminUser?.id || 0) || null;

    const insertedUsers = await db
      .insert(schema.users)
      .values({ fullname: name, role, created_by: createdBy, created_at: now, updated_at: now })
      .returning({ id: schema.users.id });
    const userId = insertedUsers?.[0]?.id;
    if (!userId) return c.json({ status: 'error', message: 'Tạo user thất bại' }, 500);
    ...
    const grantRes = await replaceUserPermissions(db, userId, permissions, createdBy ?? userId);
    if (!grantRes.ok) return c.json({ status: 'error', message: grantRes.message }, 400);

    return c.json({ id: userId, message: 'Tạo user thành công' }, 201);
  } catch (err: any) {
    return c.json({ status: 'error', message: String(err?.message || 'Internal error') }, 500);
  }
});

// Update user info + permissions (Super Admin only)
// PUT /api/admin/users/:id
app.put('/api/admin/users/:id', requireAuth, superAdminOnly, async (c) => {
  try {
    const id = Number(c.req.param('id'));
    const db = drizzle(c.env.cinema_db, { schema });
    const body = await c.req.json().catch(() => ({}));
    const name = typeof (body as any)?.name === 'string' ? String((body as any).name).trim() : undefined;
    const role = typeof (body as any)?.role === 'string' ? String((body as any).role).trim() : undefined;
    const is_active = typeof (body as any)?.is_active === 'boolean' ? (body as any).is_active : undefined;
    const permissions = Array.isArray((body as any)?.permissions) ? (body as any).permissions : undefined;

    const target = await db.query.users.findFirst({
      where: eq(schema.users.id, id),
      with: { accounts: true }
    });
    if (!target) return c.json({ status: 'error', message: 'Không tìm thấy' }, 404);
    if (target.role === 'super_admin') {
      return c.json({ status: 'error', message: 'Không thể sửa quyền của Super Admin' }, 400);
    }
    ...
    if (is_active !== undefined) {
      const accountId = target.accounts?.[0]?.id;
      if (accountId) {
        await db.update(schema.accounts).set({ is_active, updated_at: now }).where(eq(schema.accounts.id, accountId));
      }
    }

    if (permissions !== undefined) {
      const adminUser = (c as any).get('adminUser') as any;
      const grantedBy = Number(adminUser?.id || 0) || id;
      const grantRes = await replaceUserPermissions(db, id, permissions, grantedBy);
      if (!grantRes.ok) return c.json({ status: 'error', message: grantRes.message }, 400);
    }

    return c.json({ status: 'success', message: 'Cập nhật thành công' });
  } catch (err: any) {
    return c.json({ status: 'error', message: String(err?.message || 'Internal error') }, 500);
  }
});
...
app.patch('/api/admin/users/:id/toggle-active', requireAuth, superAdminOnly, async (c) => {
  try {
    const id = Number(c.req.param('id'));
    const db = drizzle(c.env.cinema_db, { schema });
    const target = await db.query.users.findFirst({
    ...
    if (!target) return c.json({ status: 'error', message: 'Không tìm thấy' }, 404);
    if (target.role === 'super_admin') {
      return c.json({ status: 'error', message: 'Không thể vô hiệu hóa Super Admin' }, 400);
    }
    const accountId = target.accounts?.[0]?.id;
    if (!accountId) return c.json({ status: 'error', message: 'Không tìm thấy account' }, 404);

    const now = new Date().toISOString();
    const nextActive = !(target.accounts?.[0]?.is_active ?? true);
    await db
...
app.delete('/api/admin/users/:id', requireAuth, superAdminOnly, async (c) => {
  try {
    const id = Number(c.req.param('id'));
    const db = drizzle(c.env.cinema_db, { schema });
    const adminUser = (c as any).get('adminUser') as any;
    if (Number(adminUser?.id) === id) {
      return c.json({ status: 'error', message: 'Không thể tự xóa chính mình' }, 400);
    }

    const target = await db.query.users.findFirst({ where: eq(schema.users.id, id) });
    if (!target) return c.json({ status: 'error', message: 'Không tìm thấy' }, 404);
    if (target.role === 'super_admin') {
      return c.json({ status: 'error', message: 'Không thể xóa Super Admin' }, 400);
    }

    await db.delete(schema.users).where(eq(schema.users.id, id));
    return c.json({ message: 'Đã xóa user' });
  } catch (err: any) {
    return c.json({ status: 'error', message: String(err?.message || 'Internal error') }, 500);
  }
});
...
app.post('/api/confirm-booking', async (c) => {
  ...
  try {
    ...
    const r = await updatePaymentImpl(db, body as any, mailer, renderBooking, tables, c.executionCtx);
    const status = typeof (r as any).status === 'number' ? (r as any).status : 200;
    const payload = {
      ...(r as any),
      status: status >= 400 ? 'error' : 'success'
...
app.post('/api/sepay/webhook', async (c) => {
  try {
    ...
    const result = await handleSePayWebhookImpl(db, tables, body, mailer, renderBooking, c.executionCtx);
...
app.post('/api/movies', async (c) => {
  try {
    ...
    const r = await createMovieImpl(db, { movies: schema.movies }, body as any, undefined, undefined, cloud.uploader);
