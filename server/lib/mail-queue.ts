...
export type EmailMetadata = {
        ...
        recipient: string;
        subject: string;
        emailType: "welcome" | "reset_password" | "booking_confirmation";
        userId?: number;
        bookingId?: number;
        provider?: string;
        additionalData?: any;
...
export class MailQueue {
        ...
        private async executeWithLogging(
                ...
                const { db, recipient, subject, emailType, userId, bookingId, provider, additionalData, emailLogsTable, runtimeEnv } = metadata;
                ...
                try {
                        if (!table) {
                                ...
                                try {
                                        const schemaModule = await import("../db/schema.js").catch(() =>
                                                import("../../worker/src/schema.js"),
                                        );
                        ...
                        console.error(
                                `[EmailTracking] ❌ Email failed (ID: ${logId}):`,
                                error?.message,
                        );
