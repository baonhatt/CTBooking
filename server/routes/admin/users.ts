...
export async function getUsersImpl(
  ...
  const itemsQuery = anyDb
    .select({
      ...
      is_active_agg:
        sql<number>`max(CASE WHEN ${tables.accounts.is_active} THEN 1 ELSE 0 END)`.as(
          "is_active_agg",
        ),
      bookings_count: sql<number>`coalesce(max(${bookingCountSq.count}), 0)`.as(
        "bookings_count",
      ),
    })
    .from(tables.users)
...
export async function getUserByIdImpl(
  anyDb: any,
  tables: { users: any; bookings: any; movies: any; ticket_packages: any },
  id: number,
) {
