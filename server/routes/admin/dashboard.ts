...
export async function getDashboardMetricsImpl(anyDb: any, tables: { movies: any; toys?: any; users: any; bookings: any; ticket_packages: any }, RUNTIME_ENV?:string, topPeriod: string = "week", year?: number) {
  ...
  const yearCondition = or(
    and(gte(tables.bookings.created_at, formatDateForDb(yearStart, RUNTIME_ENV)), lte(tables.bookings.created_at, formatDateForDb(yearEnd, RUNTIME_ENV))), 
    and(gte(tables.bookings.paid_at, formatDateForDb(yearStart, RUNTIME_ENV)), lte(tables.bookings.paid_at, formatDateForDb(yearEnd, RUNTIME_ENV)))
  );

  // Year-filtered counts based on created_at
  const [totalMoviesRes] = await anyDb.select({ count: count() })
    .from(tables.movies)
    .where(and(
  ...
  const totalToys = await (async () => {
    try {
      ...
      const [r] = await anyDb.select({ count: count() })
        .from(tables.toys)
        .where(and(
  ...
  const [totalUsersRes] = await anyDb.select({ count: count() })
    .from(tables.users)
    .where(and(
  ...
  const [totalTransactionsRes] = await anyDb.select({ count: count() }).from(tables.bookings).where(and(inArray(tables.bookings.payment_status, ["paid"]), yearCondition));
  const totalTransactions = totalTransactionsRes?.count || 0;
  
  const [revenueYearRes] = await anyDb.select({ sum: sum(tables.bookings.total_price) }).from(tables.bookings).where(and(inArray(tables.bookings.payment_status, ["paid"]), yearCondition));
  const revenueTotal = Number(revenueYearRes?.sum || 0);

  // Revenue by method (year-filtered)
  const [revenueCashYearAgg] = await anyDb.select({ sum: sum(tables.bookings.total_price) }).from(tables.bookings).where(and(inArray(tables.bookings.payment_status, ["paid"]), inArray(tables.bookings.payment_method, ["cash", "Cash"]), yearCondition));
  const [revenueMomoYearAgg] = await anyDb.select({ sum: sum(tables.bookings.total_price) }).from(tables.bookings).where(and(inArray(tables.bookings.payment_status, ["paid"]), inArray(tables.bookings.payment_method, ["momo", "MoMo"]), yearCondition));
  const [revenueVnpayYearAgg] = await anyDb.select({ sum: sum(tables.bookings.total_price) }).from(tables.bookings).where(and(inArray(tables.bookings.payment_status, ["paid"]), inArray(tables.bookings.payment_method, ["vnpay", "VNPay"]), yearCondition));
  const [revenueVietqrYearAgg] = await anyDb.select({ sum: sum(tables.bookings.total_price) }).from(tables.bookings).where(and(inArray(tables.bookings.payment_status, ["paid"]), inArray(tables.bookings.payment_method, ["vietqr", "VietQR"]), yearCondition));
  ...
  const [cashYearAgg] = await anyDb.select({ sum: sum(tables.bookings.total_price), count: count() }).from(tables.bookings).where(and(inArray(tables.bookings.payment_status, ["paid"]), inArray(tables.bookings.payment_method, ["cash", "Cash"]), yearCondition));
  const [momoYearAgg] = await anyDb.select({ sum: sum(tables.bookings.total_price), count: count() }).from(tables.bookings).where(and(inArray(tables.bookings.payment_status, ["paid"]), inArray(tables.bookings.payment_method, ["momo", "MoMo"]), yearCondition));
  const [vnpayYearAgg] = await anyDb.select({ sum: sum(tables.bookings.total_price), count: count() }).from(tables.bookings).where(and(inArray(tables.bookings.payment_status, ["paid"]), inArray(tables.bookings.payment_method, ["vnpay", "VNPay"]), yearCondition));
  const [vietqrYearAgg] = await anyDb.select({ sum: sum(tables.bookings.total_price), count: count() }).from(tables.bookings).where(and(inArray(tables.bookings.payment_status, ["paid"]), inArray(tables.bookings.payment_method, ["vietqr", "VietQR"]), yearCondition));
  ...
  const topBookings = await anyDb
    ...
    .where(and(
      ...
      or(
        and(gte(tables.bookings.created_at, formatDateForDb(topStartDate, RUNTIME_ENV)), lte(tables.bookings.created_at, formatDateForDb(topEndDate, RUNTIME_ENV))), 
        and(gte(tables.bookings.paid_at, formatDateForDb(topStartDate, RUNTIME_ENV)), lte(tables.bookings.paid_at, formatDateForDb(topEndDate, RUNTIME_ENV)))
      )
    ));
  ...
  return { 
    ...
    ticketUsage: await (async () => {
      const [used] = await anyDb.select({ count: count() }).from(tables.bookings).where(and(inArray(tables.bookings.payment_status, ["paid"]), eq(tables.bookings.is_used, true), yearCondition));
      const [total] = await anyDb.select({ count: count() }).from(tables.bookings).where(and(inArray(tables.bookings.payment_status, ["paid"]), yearCondition));
      return { used: used?.count || 0, total: total?.count || 0 };
    })(),
    paymentHealth: await (async () => {
      const [paid] = await anyDb.select({ count: count() }).from(tables.bookings).where(and(inArray(tables.bookings.payment_status, ["paid"]), yearCondition));
      const [pending] = await anyDb.select({ count: count() }).from(tables.bookings).where(and(inArray(tables.bookings.payment_status, ["pending"]), yearCondition));
      const [failed] = await anyDb.select({ count: count() }).from(tables.bookings).where(and(inArray(tables.bookings.payment_status, ["failed", "expired"]), yearCondition));
    ...
    bookingHours: await (async () => {
      ...
      const results = await anyDb.select({ createdAt: tables.bookings.created_at }).from(tables.bookings).where(and(inArray(tables.bookings.payment_status, ["paid"]), yearCondition));
...
export async function getRevenueByDateImpl(anyDb: any, tables: { bookings: any }, args: { date?: string; status?: string; year?: number }, RUNTIME_ENV?:string) {
  ...
  const [revenueVietqrAgg] = await anyDb.select({ sum: sum(tables.bookings.total_price) }).from(tables.bookings).where(and(whereCondition, inArray(tables.bookings.payment_method, ["vietqr", "VietQR"])));
...
export async function getRevenue7DaysImpl(anyDb: any, tables: { bookings: any }, RUNTIME_ENV?:string, year?: number) {
  ...
  while (currentDay <= today && currentDay <= yearEnd) {
    ...
    const [revenue] = await anyDb
      ...
      .from(tables.bookings)
      .where(and(inArray(tables.bookings.payment_status, ["paid"]), or(and(gte(tables.bookings.created_at, formatDateForDb(dayStart, RUNTIME_ENV)), lte(tables.bookings.created_at, formatDateForDb(dayEnd, RUNTIME_ENV))), and(gte(tables.bookings.paid_at, formatDateForDb(dayStart, RUNTIME_ENV)), lte(tables.bookings.paid_at, formatDateForDb(dayEnd, RUNTIME_ENV))))));
...
export async function getRevenueByMonthImpl(anyDb: any, tables: { bookings: any }, args: { year?: string; month?: string; status?: string }, RUNTIME_ENV?:string) {
  ...
  if (monthStr && yearStr) {
    ...
    const dateCondition = or(and(gte(tables.bookings.created_at, formatDateForDb(monthStart, RUNTIME_ENV)), lte(tables.bookings.created_at, formatDateForDb(monthEnd, RUNTIME_ENV))), and(gte(tables.bookings.paid_at, formatDateForDb(monthStart, RUNTIME_ENV)), lte(tables.bookings.paid_at, formatDateForDb(monthEnd, RUNTIME_ENV))));
    const statusCondition = status !== "all" ? inArray(tables.bookings.payment_status, ["paid"]) : undefined;
    ...
    const [revenueVietqrAgg] = await anyDb.select({ sum: sum(tables.bookings.total_price) }).from(tables.bookings).where(and(whereMonth, inArray(tables.bookings.payment_method, ["vietqr", "VietQR"])));
