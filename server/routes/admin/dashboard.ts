        ...
        return {
                ...
                ticketUsage: await (async () => {
                        const [used] = await anyDb
                                ...
                                .from(tables.bookings)
                                .where(
                                        and(inArray(tables.bookings.payment_status, ['paid']), eq(tables.bookings.is_used, true), yearCondition, branchConditionBookings)
                                );
                ...
                paymentHealth: await (async () => {
                        ...
                        const [failed] = await anyDb
                                .select({ count: count() })
                                .from(tables.bookings)
                                .where(and(inArray(tables.bookings.payment_status, ['failed', 'expired']), yearCondition, branchConditionBookings));
