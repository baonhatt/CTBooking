...
export async function getToyImpl(anyDb: any, tables: { toys: any; auditLogs: any }, id: number) {
        ...
        const [createLog] = await anyDb
                ...
                .where(and(eq(tables.auditLogs.entityType, 'toy'), eq(tables.auditLogs.entityId, String(id)), eq(tables.auditLogs.action, 'create')))
        ...
        const [updateLog] = await anyDb
                .select()
                .from(tables.auditLogs)
                .where(and(eq(tables.auditLogs.entityType, 'toy'), eq(tables.auditLogs.entityId, String(id)), eq(tables.auditLogs.action, 'update')))
