...
export async function listActiveTicketPackages(anyDb: any, tables: { ticket_packages: any; movies: any }, branch_id?: number) {
        const whereCondition = branch_id
                ? and(eq(tables.ticket_packages.is_active, true), eq(tables.ticket_packages.branch_id, branch_id))
