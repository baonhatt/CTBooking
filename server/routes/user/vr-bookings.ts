        ...
        try {
                ...
                const itemsToSave: VRPackageLineItem[] = validated.vr_items_resolved.map((it) => {
                        const disc_unit = voucher_applied
                                ? it.unit_price - (validated.voucher_discount_amount > 0
                                        ? +((it.unit_price * it.quantity) * validated.voucher_discount_amount / validated.original_total_price / it.quantity).toFixed(2)
                                        : 0)
                                : it.unit_price;
