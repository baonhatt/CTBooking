...
export const createToy: RequestHandler = async (req, res) => {
  try {
    const { name, category, price, stock, status, image_url, image_base64 } =
      req.body as any;
    ...
    res.status(201).json({ message: "Thêm đồ chơi mới thành công", toy });
  } catch (err: any) {
...
export const updateToy: RequestHandler = async (req, res) => {
  try {
    ...
    res.status(200).json({ message: "Cập nhật đồ chơi thành công", toy });
  } catch (err: any) {
    if (err?.code === "P2025")
...
export const deleteToy: RequestHandler = async (req, res) => {
  try {
    ...
    res.status(200).json({ message: "Xóa đồ chơi thành công", ok: true });
  } catch (err: any) {
    if (err?.code === "P2025")
      return res.status(404).json({ message: "Không tìm thấy" });
    res.status(500).json({ message: "Lỗi máy chủ nội bộ" });
